...
:::{region} native-library-reference/kernel
...
:::{region} http-member-conventions/kernel
...
:::{region} native-library-trainable-backends/description-delivery
...
- **Config extras (one open table — hashed, engine-opaque):** each member's
  ...
  :::{region} config-extras/reserved-keys-disjoint
...
:::{transclude} the-aggregator-pattern/division-of-labor
:::
