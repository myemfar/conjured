...
:::{transclude} pipeline-result-runresult/shape
:::
...
:::{region} run-trigger-correlation/history-less-hub
...
- `data:` — the event's payload as a JSON object, **the canonical in-process payload** for that
  ...
  :::{region} event-stream-frames/data-null-serialization
...
:::{region} event-stream-frames/run-scoped-completeness
...
:::{transclude} orchestration-scope/no-partial-values
:::
